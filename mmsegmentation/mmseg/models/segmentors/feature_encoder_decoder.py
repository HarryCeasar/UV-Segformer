...
class FeatureEncoderDecoder(BaseSegmentor):
    ...
    def extract_feat(self, inputs: Tensor) -> List[Tensor]:
        ...
        x1 = self.backbone1(inputs1)
        x2 = self.backbone2(inputs2)
        x3 = self.backbone3(inputs3)
        x = [torch.cat((out1, out2, out3), dim=1) for out1, out2, out3 in zip(x1, x2, x3)]
        if self.with_neck:
            x = self.neck(x)
        return x
