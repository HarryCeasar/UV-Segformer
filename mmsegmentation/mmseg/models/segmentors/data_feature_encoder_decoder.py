...
class DataFeatureEncoderDecoder(BaseSegmentor):
    ...
    def extract_feat(self, inputs: Tensor) -> List[Tensor]:
        ...
        spectra_x = self.backbone(spectra_inputs)
        building_x = self.backbone(building_inputs)
        x = [torch.cat((out1, out2), dim=1) for out1, out2 in zip(spectra_x, building_x)]
        if self.with_neck:
            x = self.neck(x)
        return x
