...
@TRANSFORMS.register_module()
class PhotoMetricDistortionLimitedChannels(BaseTransform):
    """Apply photometric distortion only to the first 3 channels of an image sequentially,
    every transformation is applied with a probability of 0.5. The position of random contrast
    is in second or second to last.
    """
    ...
    def convert(self,
                ...
                alpha: int = 1,
                beta: int = 0) -> np.ndarray:
        """Only modify the first 3 channels with alpha and beta, with clipping."""
        img[..., :3] = img[..., :3].astype(np.float32) * alpha + beta
        img[..., :3] = np.clip(img[..., :3], 0, 255)
        return img.astype(np.uint8)

    def brightness(self, img: np.ndarray) -> np.ndarray:
        """Apply brightness distortion only to the first 3 channels."""
        if random.randint(2):
            return self.convert(
                img,
    ...
    def contrast(self, img: np.ndarray) -> np.ndarray:
        """Apply contrast distortion only to the first 3 channels."""
        if random.randint(2):
            return self.convert(
                img,
                alpha=random.uniform(self.contrast_lower, self.contrast_upper))
    ...
    def saturation(self, img: np.ndarray) -> np.ndarray:
        """Apply saturation distortion only to the first 3 channels."""
        if random.randint(2):
            img_hsv = mmcv.bgr2hsv(img[..., :3])
            img_hsv[:, :, 1] = self.convert(
                img_hsv[:, :, 1],
                alpha=random.uniform(self.saturation_lower, self.saturation_upper))
            img[..., :3] = mmcv.hsv2bgr(img_hsv)
        return img

    def hue(self, img: np.ndarray) -> np.ndarray:
        """Apply hue distortion only to the first 3 channels."""
        if random.randint(2):
            img_hsv = mmcv.bgr2hsv(img[..., :3])
            img_hsv[:, :, 0] = (img_hsv[:, :, 0].astype(int) + random.randint(-self.hue_delta, self.hue_delta)) % 180
            img[..., :3] = mmcv.hsv2bgr(img_hsv)
        return img

    def transform(self, results: dict) -> dict:
        """Perform photometric distortion on the first 3 channels of images."""
        img = results['img']
        # random brightness
        img = self.brightness(img)
